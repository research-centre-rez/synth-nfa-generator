...
class MitsubaElement:
    ...
    def to_dict(self):
        ...
        return {self.element_key: inner_out_dict}
...
class MitsubaScene:
    ...
    def render(
        ...
        cam_key = next(iter(label_sensor_dict.keys()))
        label_sensor_dict[cam_key]["film"]["rfilter"] = {"type": "box"}
        # HACK (IN A SCENE WHERE CAMERA IS SUBMERGED) removing the reference to 
        # the medium from camera will
        ...
        cam_key = next(iter(beauty_sensor_dict.keys()))
        ...
        if rodgrid_labels:
            aovintegrator = mi.load_dict(
                {"type": "aov", "aovs": "si:shape_index", "beauty": {"type": "path"}}
            )
            rendered_label = mi.render(
                scene, spp=1, sensor=label_sensor, integrator=aovintegrator
            )
        ...
        if oxide_labels:
            ...
            lrend = mi.render(
                scene, spp=8, integrator=path_integrator, sensor=label_sensor
            )
            oxide_mask = np.all(
                np.clip(lrend[:, :, :3], 0, 1) == np.array(pink)[None, None], axis=2
            )
        ...
        if denoise:
            denoiser = mi.OptixDenoiser(
                input_size=[res_x, res_y], albedo=False, normals=False, temporal=False
            )
    ...
    def from_inspection_scene(inspection_scene: ss.InspectionScene, krng, *keys):
        ...
        if nfa_spec is not None:
            ...
            grids = [
                MitsubaElement(
                    ...
                    mitsuba_grid(
                        grid_spec,
                        nfa_spec.rods_shape_spec,
                        material_id=grids_material_id,
                    ),
                )
                for i, grid_spec in enumerate(nfa_spec.grids_specs)
        ...
        if (
            isinstance(rods_material, ss.MaterialOxidizedConductorSpec)
            and rods_material.oxide_spots_spec is not None
        ):
            oxide_spots_spec = rods_material.oxide_spots_spec

            krng.uint32("poisson_seed", *keys) // 2
            ...
            for i, rod_dict in enumerate(rods[:rod_count]):
                ...
                rod_height = np.abs(
                    rod_dict.element_dict["p0"][2] - rod_dict.element_dict["p1"][2]
                )
            ...
            for r, rod_texture_res in zip(rods, oxide_bank.values()):
                ...
                if oxide_spots_spec.blur_sigma > 1:
                    oxide_mask = ndi.gaussian_filter(
                        oxide_mask, sigma=oxide_spots_spec.blur_sigma
                    )

                rod_material_weight = (
                    (oxide_mask[:, :, None]) * oxide_spots_spec.opacity
                )
                ...
                new_rod = MitsubaRodElement(
                    r.element_key, new_dict, oxide_texture=rod_material_weight
                )
        ...
        main_cam = MitsubaElement(
            "main_camera",
            {
                ...
                "film": {
                    ...
                    "rfilter": {
                        "type": "box"
                    },  # <- TODO: this is not a default filter but I don't know why it was chosen.
                },
            },
        )
            ...
            MitsubaElement(
                f"emitter_{i:05}",
                {
                    ...
                    "to_world": (
                        ...
                        # size is 2x2
                        .scale([es.panel_width / 2, es.panel_height / 2, 1])
                    ),
                    "emitter": {
                        "type": "area",
                        "radiance": {
        ...
        medium = None
        if (
            inspection_scene.medium_spec is not None
            and inspection_scene.medium_spec.enabled
        ):
            ...
            medium_content = MitsubaElement("medium", medium_content_dict)
            medium_boundaries = MitsubaElement(
                "medium_boundaries", medium_boundaries_dict
            )
...
class PostProcessRenderingVariation:
    noise_denoise_blend: sv.MultiplicativeScalarUniformVariationSpecs = (
        sv.MultiplicativeScalarUniformVariationSpecs()
    )
    ...
    def sample(self, render_result: RenderResult, krng, *keys):
        ...
        return (
            alpha * render_result.denoised_rgba + (1 - alpha) * render_result.raw_rgba
        )
...
def inconel_conductor(alpha_u=None, alpha_v=None):
    ...
    return create_manual_conductor(
        eta_spectrum, k_spectrum, alpha_u=alpha_u, alpha_v=alpha_v
    )
...
def traverse_oxide_label_scene(
    ...
    for r in mitsuba_scene.rods:
        ...
        if r.oxide_texture is not None:
            ...
            texture_tensor = mi.TensorXf(
                np.dstack([oxide_texture] * 3)
                * label_color_3ch
                * label_emitter_intensity
            )
...
def convert_to_mitsuba_material(material_definition, material_id=None):
    ...
    if isinstance(material_definition, ss.MaterialOxidizedConductorSpec):
        ...
        definition = {
            ...
            "rods": convert_to_mitsuba_material(
                material_definition.conductor_spec, material_id=None
            ),
            "oxidation": convert_to_mitsuba_material(
                material_definition.oxidation_spec, material_id=None
            ),
        }
        ...
        if material_definition.conductor_name == "custom_Zircon":
            ...
            definition = {"material": material_definition.conductor_name}
            if material_definition.rough_conductor_spec is None:
        ...
        definition = {"type": "diffuse", "reflectance": {"type": "rgb", "value": rgb}}
    else:
        raise TypeError("Material is not known type")
...
def create_medium(
    ...
    if volume_spec.centered_to_origin:
        grid_to_world = (
            mi.ScalarTransform4f.translate([-volume_spec.cube_width / 2] * 3)
            @ grid_to_world
        )
    ...
    medium_content = {
        ...
        "phase": {"type": "hg", "g": heterogenous_medium_spec.hg_phase_g},
...
def unsafe_read_labels(label_instances, clean_jitter=False):
    ...
    _h, w = rods_instance_mask.shape
    masks = np.array(
        [
            (label_instances == val) * (i + 1)
            for i, val in enumerate(unique_vals)
            if val != 0
        ]
    )
    widths = np.array([(b[3] - b[2]) for b in [bbox(m) for m in masks]])
    np.array([(b[1] - b[0]) for b in [bbox(m) for m in masks]])
    ...
    possible_grids = np.argwhere(
        (widths > label_instances.shape[1] * 0.5) & (widths < w)
    )
    if len(possible_grids) == 1:
        ...
        n, arr, stats, _ = cv2.connectedComponentsWithStats(np.uint8(grid_mask))
        ...
        n, arr, stats, _ = cv2.connectedComponentsWithStats(
            np.uint8(rods_instance_mask > 0)
        )
        np.array([(arr == mask_idx) for mask_idx in range(1, n)])
        ws = stats[:, cv2.CC_STAT_WIDTH].astype(np.float32)
        # rods is anything above scaled average
        ws[(ws == arr.shape[1])] = np.nan
        rod_cmp_filter = ws > np.nanmean(ws) * 0.8
