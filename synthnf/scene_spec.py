...
class TextureOxideSpotsSpec:
    ...
    opacity: float = 0.25
    poisson_disk_radius: float = 0.04
    blur_sigma: float = 0
...
class MaterialOxidizedConductorSpec:
    ...
    oxidation_amount: float = 0.1
    oxide_spots_spec: TextureOxideSpotsSpec | None = None
...
class HeterogenousMediumSpec:
    ...
    albedo: MaterialBSDFSpec = MaterialBSDFSpec(r=0.8, g=0.85, b=0.95)
    hg_phase_g: float = 0.75
    scale: float = 1
...
class RodsHexagonSpec:
    ...
    def collect_rods(self) -> tuple[FuelRodSpec]:
        ...
        return tuple(
            [
                FuelRodSpec(
                    xyz=np.array(xyz), radius=self.rod_radius, height=self.rod_height
                )
                for xyz in xyzs
            ]
        )
...
class RodsSquareSpec:
    ...
    def collect_rods(self) -> tuple[FuelRodSpec]:
        ...
        return tuple(
            [
                FuelRodSpec(xyz=xyz, radius=self.rod_radius, height=self.rod_height)
                for xyz in xyzs
            ]
        )
...
class NfaSpec:
    ...
    def from_shape(
        rods_shape_spec,
        rods_material_spec,
        grids: list[SpacerGridSpec] | None = None,
        grids_material_spec: MaterialSpec | None = None,
    ):
        if grids is None:
            grids = []
...
class GenericCameraRingSpec:
    ...
    def single_cam_two_panel_light(
        ...
        emitter_left = PanelEmitterSpec(
            lookat_origin_xyz=(x - light_offset / 2, y, z), intensity=light_intensity
        )
        emitter_right = PanelEmitterSpec(
            lookat_origin_xyz=(x + light_offset / 2, y, z), intensity=light_intensity
        )
        main_cam = PerspectiveSensorSpec(
            lookat_origin_xyz=(x, y, z), field_of_view=field_of_view
        )
        return GenericCameraRingSpec(
            sensor_specs=[main_cam], emitter_specs=[emitter_left, emitter_right]
        )
...
class AhlbergCameraRingSpec:
    ...
    def single_cam_four_x_lights(
        ...
        return GenericCameraRingSpec(sensor_specs=[main_cam], emitter_specs=emmiters)
...
def array_rectangle_grid(
    column_number, row_number, offset_rows=1, offset_columns=1, center=True
):
