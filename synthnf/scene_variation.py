...
class KeyedRNG:
    ...
    def pythonint(self, *keys) -> int:
        return stable_hash(self.root_seed, *keys, digest_size=self._digest_size)

    def uint32(self, *keys) -> int:
        return stable_hash(self.root_seed, *keys, digest_size=8) % (2**32)
    
    def int32(self, *keys) -> int:
        u32 = stable_hash(self.root_seed, *keys, digest_size=8) % (2**32)
        return u32 - (2**31)
    ...
    def uniform_list(self, size, low=0, high=1, *keys) -> list[float]:
        return np.array([self.uniform(low, high, i, *keys) for i in np.arange(size)])
...
class FuelRodVariationParams:
    ...
    radius: MultiplicativeScalarUniformVariationSpecs = (
        MultiplicativeScalarUniformVariationSpecs()
    )
    height: MultiplicativeScalarUniformVariationSpecs = (
        MultiplicativeScalarUniformVariationSpecs()
    )
...
class SpacerGridVariation:
    ...
    z_location: AdditiveScalarUniformVariationSpecs = (
        AdditiveScalarUniformVariationSpecs()
    )
    ...
    def sample(self, grid_spec: ss.SpacerGridSpec, krng, *keys):
        return replace(
            grid_spec,
            z_location=self.z_location.sample(
                grid_spec.z_location, krng, "z_loc", *keys
            ),
            enabled=self.enabled.sample(None, krng, "enabled", *keys),
            tooth_ply_filename=self.tooth_ply_filename.sample(
                grid_spec.tooth_ply_filename, krng, "tooth_ply", *keys
            ),
        )
...
class MaterialRoughConductorVariation:
    alpha_u: MultiplicativeScalarUniformVariationSpecs = (
        MultiplicativeScalarUniformVariationSpecs()
    )
    alpha_v: MultiplicativeScalarUniformVariationSpecs = (
        MultiplicativeScalarUniformVariationSpecs()
    )

    def sample(
        self, rough_conductor_spec: ss.MaterialRoughConductorSpec | None, krng, *keys
    ):
...
class MaterialConductorVariation:
    ...
    rough_conductor_variation: MaterialRoughConductorVariation = (
        MaterialRoughConductorVariation()
    )

    def sample(
        self, rods_material_spec: ss.MaterialSpec, krng, *keys
    ) -> ss.MaterialSpec:
        if isinstance(rods_material_spec, ss.MaterialNamedAnyConductorSpec):
            return replace(
                rods_material_spec,
                conductor_name=self.conductor_name.sample(
                    rods_material_spec, krng, "name", *keys
                ),
                rough_conductor_spec=self.rough_conductor_variation.sample(
                    rods_material_spec.rough_conductor_spec, krng, "rough", *keys
                ),
            )
        else:
...
class TextureOxideSpotsVariation:
    noise_texture_zoom: MultiplicativeScalarUniformVariationSpecs = (
        MultiplicativeScalarUniformVariationSpecs()
    )
    min_oxide_size_px: MultiplicativeScalarUniformVariationSpecs = (
        MultiplicativeScalarUniformVariationSpecs()
    )
    max_oxide_size_px: MultiplicativeScalarUniformVariationSpecs = (
        MultiplicativeScalarUniformVariationSpecs()
    )
    oxide_spots_coverage: MultiplicativeScalarUniformVariationSpecs = (
        MultiplicativeScalarUniformVariationSpecs()
    )
    opacity: MultiplicativeScalarUniformVariationSpecs = (
        MultiplicativeScalarUniformVariationSpecs()
    )
    poisson_disk_radius: MultiplicativeScalarUniformVariationSpecs = (
        MultiplicativeScalarUniformVariationSpecs()
    )
    blur_sigma: MultiplicativeScalarUniformVariationSpecs = (
        MultiplicativeScalarUniformVariationSpecs()
    )
    ...
    def sample(self, oxide_spots_spec: ss.TextureOxideSpotsSpec, krng, *keys):
        ...
        return replace(
            oxide_spots_spec,
            noise_texture_zoom=self.noise_texture_zoom.sample(
                oxide_spots_spec.noise_texture_zoom, krng, "noise_texture_zoom", *keys
            ),
            min_oxide_size_px=self.min_oxide_size_px.sample(
                oxide_spots_spec.min_oxide_size_px, krng, "min_oxide_size_px", *keys
            ),
            max_oxide_size_px=self.max_oxide_size_px.sample(
                oxide_spots_spec.max_oxide_size_px, krng, "max_oxspx", *keys
            ),
            oxide_spots_coverage=self.oxide_spots_coverage.sample(
                oxide_spots_spec.oxide_spots_coverage, krng, "oxsc", *keys
            ),
            opacity=self.opacity.sample(
                oxide_spots_spec.opacity, krng, "opacity", *keys
            ),
            poisson_disk_radius=self.poisson_disk_radius.sample(
                oxide_spots_spec.poisson_disk_radius, krng, "pdr", *keys
            ),
            blur_sigma=self.blur_sigma.sample(
                oxide_spots_spec.blur_sigma, krng, "blur_s", *keys
            ),
        )
...
class MaterialOxidizedConductorVariation:
    ...
    oxidation_amount: MultiplicativeScalarUniformVariationSpecs = (
        MultiplicativeScalarUniformVariationSpecs(1, 1)
    )
    ...
    def sample(self, oxidized_conductor: ss.MaterialOxidizedConductorSpec, krng, *keys):
        ox = self.oxidation_amount.sample(
            oxidized_conductor.oxidation_amount, krng, "oxide_amount", *keys
        )
        ox = np.clip(ox, 0, 1)

        return replace(
            oxidized_conductor,
            conductor_spec=self.conductor_variation.sample(
                oxidized_conductor.conductor_spec, krng, "cond_spec", *keys
            ),
            oxidation_amount=ox,
            oxide_spots_spec=self.oxide_spots_spec.sample(
                oxidized_conductor.oxide_spots_spec, krng, "spot_oxidation", *keys
            ),
        )
...
class NfaVariationParams:
    rods_variations: tuple[
        FuelRodVariationParams, ...
    ] = ()  # Variation per rod, not ideal at the moment
    grids_variations: tuple[SpacerGridVariation, ...] = ()
    rods_material_variation: MaterialOxidizedConductorVariation = (
        MaterialOxidizedConductorVariation()
    )
    grids_material_variation: MaterialOxidizedConductorVariation = (
        MaterialOxidizedConductorVariation()
    )
    ...
    def sample(self, nfa_spec: ss.NfaSpec, krng, *keys):
        ...
        return replace(
            ...
            rods_specs=[
                ...
                for i, (spec, var) in zip_specifications_w_variations(
                    nfa_spec.rods_specs, self.rods_variations
                )
            ],
            grids_specs=[
                var.sample(spec, krng, f"grid_{i}", "grids_spec", *keys)
                for i, (spec, var) in zip_specifications_w_variations(
                    nfa_spec.grids_specs, self.grids_variations
                )
            ],
...
class EmitterVariation:
    ...
    intensity: MultiplicativeScalarUniformVariationSpecs = (
        MultiplicativeScalarUniformVariationSpecs()
    )
    ...
    panel_width: MultiplicativeScalarUniformVariationSpecs = (
        MultiplicativeScalarUniformVariationSpecs()
    )
    panel_height: MultiplicativeScalarUniformVariationSpecs = (
        MultiplicativeScalarUniformVariationSpecs()
    )

    def sample(
        self, emitter_spec: ss.PanelEmitterSpec, krng, *keys
    ) -> ss.PanelEmitterSpec:
        new_lookat_origin_xyz = tuple(
            self.lookat_origin_xyz.sample(
                emitter_spec.lookat_origin_xyz, krng, "lookat_origin_xyz", *keys
            )
        )
        new_lookat_up_xyz = tuple(
            self.lookat_up_xyz.sample(
                emitter_spec.lookat_up_xyz, krng, "lookat_up_xyz", *keys
            )
        )
        new_lookat_target_xyz = tuple(
            self.lookat_target_xyz.sample(
                emitter_spec.lookat_target_xyz, krng, "lookat_target_xyz", *keys
            )
        )
        return replace(
            emitter_spec,
            lookat_origin_xyz=new_lookat_origin_xyz,
            intensity=self.intensity.sample(
                emitter_spec.intensity, krng, "intensity", *keys
            ),
            lookat_up_xyz=new_lookat_up_xyz,
            lookat_target_xyz=new_lookat_target_xyz,
            panel_width=self.panel_width.sample(
                emitter_spec.panel_width, krng, "panel_width", *keys
            ),
            panel_height=self.panel_height.sample(
                emitter_spec.panel_height, krng, "panel_height", *keys
            ),
        )
...
class PerspectiveSensorVariation:
    ...
    field_of_view: MultiplicativeScalarUniformVariationSpecs = (
        MultiplicativeScalarUniformVariationSpecs()
    )

    def sample(
        self, sensor_spec: ss.PerspectiveSensorSpec, krng, *keys
    ) -> ss.PerspectiveSensorSpec:
        new_lookat_origin_xyz = tuple(
            self.lookat_origin_xyz.sample(
                sensor_spec.lookat_origin_xyz, krng, "lookat_origin_xyz", *keys
            )
        )
        new_lookat_up_xyz = tuple(
            self.lookat_up_xyz.sample(
                sensor_spec.lookat_up_xyz, krng, "lookat_up_xyz", *keys
            )
        )
        new_lookat_target_xyz = tuple(
            self.lookat_target_xyz.sample(
                sensor_spec.lookat_target_xyz, krng, "lookat_target_xyz", *keys
            )
        )
        return replace(
            sensor_spec,
            ...
            lookat_up_xyz=new_lookat_up_xyz,
            lookat_target_xyz=new_lookat_target_xyz,
            field_of_view=self.field_of_view.sample(
                sensor_spec.field_of_view, krng, "fov", *keys
            ),
        )
    ...
    def sample(self, cam_ring_spec: ss.GenericCameraRingSpec, krng, *keys):
        return replace(
            ...
            emitter_specs=[
                ...
                for i, (spec, var) in zip_specifications_w_variations(
                    cam_ring_spec.emitter_specs, self.emitter_variations
                )
            ],
            sensor_specs=[
                var.sample(spec, krng, f"sensor_{i}", *keys)
                for i, (spec, var) in zip_specifications_w_variations(
                    cam_ring_spec.sensor_specs, self.sensor_variation
                )
            ],
        )
...
class EnvironmentMapVariationParams:
    intensity_scale: MultiplicativeScalarUniformVariationSpecs = (
        MultiplicativeScalarUniformVariationSpecs()
    )
    env_map_file: ChoiceVariationParams = ChoiceVariationParams()
    rot_left_right_angle: AdditiveScalarUniformVariationSpecs = (
        AdditiveScalarUniformVariationSpecs(0, 0)
    )

    def sample(self, env_map_spec: ss.EnvironmentMapSpec, krng, *keys):
        intensity = self.intensity_scale.sample(
            env_map_spec.intensity_scale, krng, "intensity", *keys
        )
        intensity = np.maximum(intensity, 0)
        return replace(
            env_map_spec,
            intensity_scale=intensity,
            env_map_file=self.env_map_file.sample(
                "DUMMY NON USED VALUE", krng, "env_map", *keys
            ),
            rot_left_right_angle=self.rot_left_right_angle.sample(
                env_map_spec.rot_left_right_angle, krng, "lr_rot", *keys
            ),
        )
...
class GlobalIlluminationVariation:
    ...
    def sample(self, global_illumination_spec: ss.GlobalIlluminationSpec, krng, *keys):
        ...
        return replace(
            ...
            intensity=self.intensity.sample(
                global_illumination_spec.intensity, krng, "gl_intensity", *keys
            ),
        )
...
class HeterogenousMediumVariation:
    ...
    hg_phase_g: MultiplicativeScalarUniformVariationSpecs = (
        MultiplicativeScalarUniformVariationSpecs()
    )
    scale: MultiplicativeScalarUniformVariationSpecs = (
        MultiplicativeScalarUniformVariationSpecs()
    )
    ...
    def sample(self, spec: ss.HeterogenousMediumSpec, krng, *keys):
        ...
        if not isinstance(spec, ss.HeterogenousMediumSpec):
            raise TypeError(
                f"Heterogenous Medium variaion canot vary specification of type {type(spec)}"
            )
        ...
        return replace(
            spec,
            albedo=albedo_bsdf,
            hg_phase_g=self.hg_phase_g.sample(
                spec.hg_phase_g, krng, "hg-phase_g", *keys
            ),
...
class InspectionVariationParams:
    ...
    global_illumination_variation: GlobalIlluminationVariation = (
        GlobalIlluminationVariation()
    )
    ...
    def sample(self, inspection_scene: ss.InspectionScene, krng, *keys):
        return replace(
            ...
            global_illumination_spec=self.global_illumination_variation.sample(
                inspection_scene.global_illumination_spec, krng, "gl", *keys
            ),
            medium_spec=self.medium_variation.sample(
                inspection_scene.medium_spec, krng, "medium", *keys
            ),
        )
