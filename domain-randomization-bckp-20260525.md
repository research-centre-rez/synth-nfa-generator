...
inspection_variation = sv.InspectionVariationParams(
    nfa_variation=sv.NfaVariationParams(
        rods_variations=[
            sv.FuelRodVariationParams(
                radius=sv.MultiplicativeScalarUniformVariationSpecs(
                    low=0.95, high=1.05
                ),
                height=sv.MultiplicativeScalarUniformVariationSpecs(low=0.9, high=1.1),
        ...
        * 100,
        grids_variations=[
            sv.SpacerGridVariation(
                ...
                enabled=sv.ChanceBoolVariation(success_probability=0.2),
        ...
        rods_material_variation=sv.MaterialOxidizedConductorVariation(
            conductor_variation=sv.MaterialConductorVariation(
                ...
                rough_conductor_variation=sv.MaterialRoughConductorVariation(
                    alpha_u=sv.MultiplicativeScalarUniformVariationSpecs(0.8, 1.2),
                    alpha_v=sv.MultiplicativeScalarUniformVariationSpecs(0.8, 1.2),
                ),
            ),
            oxidation_amount=sv.AdditiveScalarUniformVariationSpecs(0, 0.2),
            oxide_spots_spec=sv.TextureOxideSpotsVariation(
                oxide_spots_coverage=sv.AdditiveScalarUniformVariationSpecs(
                    low=-0.25, high=0.25
                ),
                poisson_disk_radius=sv.AdditiveScalarUniformVariationSpecs(
                    low=-0.01, high=0.02
                ),
                opacity=sv.AdditiveScalarUniformVariationSpecs(low=-0.1, high=0.05),
        ...
        grids_material_variation=sv.MaterialOxidizedConductorVariation(
            ...
            oxidation_amount=sv.AdditiveScalarUniformVariationSpecs(0, 0.1),
        ),
    ),
    cam_ring_variation=sv.CamRingVariationParams(
        emitter_variations=[
            sv.EmitterVariation(
                intensity=sv.MultiplicativeScalarUniformVariationSpecs(
                    low=0.8, high=1.2
                ),
        ...
        * 2,
        sensor_variation=[
            sv.PerspectiveSensorVariation(
                field_of_view=sv.MultiplicativeScalarUniformVariationSpecs(
                    low=0.95, high=1.05
                ),
                lookat_target_xyz=sv.XyzUniformVariationSpecs(
    ...
    env_map_variation=sv.EnvironmentMapVariationParams(
        intensity_scale=sv.MultiplicativeScalarUniformVariationSpecs(0.85, 1.1),
    ...
    medium_variation=sv.HeterogenousMediumVariation(
        ...
        # ) ,
        albedo=sv.MultiplicativeScalarUniformVariationSpecs(0.1, 1.2),
        hg_phase_g=sv.MultiplicativeScalarUniformVariationSpecs(0.1, 1),
        scale=sv.MultiplicativeScalarUniformVariationSpecs(1, 10),
        enabled=sv.ChanceBoolVariation(success_probability=0.60),
    ),
)
```
...
scene_dict["big_ball"] = {
    "type": "sphere",
    "to_world": mi.ScalarAffineTransform4f().scale(
        [150, 150, 150]
    ),  # .translate([0, 0, 0]),
    "bsdf": {"type": "diffuse"},
}
